...
<div class="wd-page">
    ...
    <div class="wd-balance-card">
        ...
        <div class="wd-min">Minimum withdrawal: <strong>&#x20A6;{{ number_format($minimum_withdrawal, 2) }}</strong></div>
    </div>
    ...
    <div class="wd-form-card">
        ...
        <form id="withdrawForm" method="POST" action="{{ route('withdraw.process') }}" autocomplete="off" novalidate>
            @csrf
            ...
            <div id="default_bank_group" class="wd-bank-info">
                <div class="wd-bank-row"><span>Bank</span><span class="wd-bank-val">{{ $bank_details['bank_name'] }}</span></div>
                <div class="wd-bank-row"><span>Account Number</span><span class="wd-bank-val">{{ $bank_details['account_number'] }}</span></div>
                <div class="wd-bank-row"><span>Account Name</span><span class="wd-bank-val">{{ $bank_details['account_name'] }}</span></div>
            </div>
...
(function () {
    ...
    function triggerVerify() {
        ...
        fetch(`{{ route("ajax.verify-account") }}?account_number=${encodeURIComponent(acctNum)}&bank_code=${encodeURIComponent(bankCode)}`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(r => r.json())
        .then(data => {
    ...
    function validate() {
        ...
        if (boEl.value === 'default') {
            const bname = '{{ $bank_details["bank_name"] }}';
            if (!bname || bname === 'N/A') {
                showToast('Default bank not set. Redirecting to settings…', 'error');
                setTimeout(() => { window.location.href = '{{ route("edit-bank") }}'; }, 2500);
                return false;
            }
        }
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            const res  = await fetch('{{ route("withdraw.process") }}', {
                method: 'POST', body: fd,
