                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex justify-content-between">
                        <div>
                            ...
                            <div class="nx-stat-value" id="totalNotifications">{{ $notifications->total() }}</div>
                        </div>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex justify-content-between">
                        <div>
                            ...
                            <div class="nx-stat-value" id="unreadCount">{{ $notifications->where('is_read', false)->count() }}</div>
                        </div>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex justify-content-between">
                        <div>
                            ...
                            <div class="nx-stat-value" id="todayCount">{{ $notifications->where('created_at', '>=', today())->count() }}</div>
                        </div>
                    </div>
                ...
                <div class="card-body p-0">
                    @if($notifications->count() > 0)
                        <div class="nx-list">
                            ...
                            @endphp
                            <div class="nx-item notification-item {{ $isUnread ? 'nx-unread' : '' }}"
                                 style="--i: {{ $loop->index }}"
                                 data-id="{{ $notification->id }}"
                                 data-read="{{ $notification->is_read ? 'true' : 'false' }}"
                                 data-title="{{ e($notification->title) }}"
                                 data-message="{{ e($notification->message) }}"
                                 data-time="{{ e($notification->time_ago) }}"
                                 data-system="{{ $notification->is_broadcast ? '1' : '0' }}"
                                 data-expires="{{ $notification->expires_at && $notification->expires_at->isFuture() ? e('Expires '.$notification->expires_at->diffForHumans()) : '' }}">
                                <div class="nx-bubble-icon" style="color: {{ $iconColor }}">
                                    <i class="bi {{ $notification->icon }}"></i>
                                </div>
                                <div class="nx-main">
                                    <div class="nx-top">
                                        <h6 class="nx-h">{{ $notification->title }}</h6>
                                        <div class="nx-actions">
                                            @if(!$notification->is_read)
                                            <button class="nx-icon-btn read mark-read" data-id="{{ $notification->id }}" title="Mark as read">
                                                <i class="bi bi-check2"></i>
                                            </button>
                                            @endif
                                            <button class="nx-icon-btn del delete-notification" data-id="{{ $notification->id }}" title="Delete notification">
                                                <i class="bi bi-trash3"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <p class="nx-msg">{{ \Illuminate\Support\Str::limit($notification->message, 110) }}</p>
                                    <div class="nx-meta">
                                        ...
                                        @if($notification->expires_at && $notification->expires_at->isFuture())
                                            <span class="nx-pill exp">Expires {{ $notification->expires_at->diffForHumans() }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const unreadCountUrl = @json(route('notifications.unread-count'));
