...
@section('title', 'About Us — KayXchange')
...
<div class="about-hero">
    ...
    <h1>{{ $sc['about_page_hero_title'] ?? 'About KayXchange' }}</h1>
    <p>{{ $sc['about_page_hero_subtitle'] ?? "Nigeria's most trusted platform for buying &amp; selling digital assets." }}</p>
</div>
...
<div class="about-wrap">
    ...
    <div class="ab-stats mb-4">
        <div class="ab-stat">
            <div class="ab-stat-num">{{ $sc['about_page_stat1_num'] ?? '3,000+' }}</div>
            <div class="ab-stat-lbl">{{ $sc['about_page_stat1_label'] ?? 'Happy Clients' }}</div>
        </div>
        <div class="ab-stat">
            <div class="ab-stat-num">{{ $sc['about_page_stat2_num'] ?? '90,000+' }}</div>
            <div class="ab-stat-lbl">{{ $sc['about_page_stat2_label'] ?? 'Trades Completed' }}</div>
        </div>
        <div class="ab-stat">
            <div class="ab-stat-num">{{ $sc['about_page_stat3_num'] ?? '24/7' }}</div>
            <div class="ab-stat-lbl">{{ $sc['about_page_stat3_label'] ?? 'Customer Support' }}</div>
        </div>
        <div class="ab-stat">
            <div class="ab-stat-num">{{ $sc['about_page_stat4_num'] ?? '15+' }}</div>
            <div class="ab-stat-lbl">{{ $sc['about_page_stat4_label'] ?? 'Team Members' }}</div>
        </div>
    </div>

    {{-- Our Story --}}
    <div class="ab-section-title"><i class="bi bi-journal-text"></i>{{ $sc['about_page_story_heading'] ?? 'Our Story' }}</div>
    <div class="ab-story mb-4">
        <p>{{ $sc['about_page_story_text'] ?? '' }}</p>
    </div>
    ...
    <div class="ab-mv-grid">
        <div class="ab-mv-card">
            ...
            <h3>{{ $sc['about_page_mission_heading'] ?? 'Our Mission' }}</h3>
            <p>{{ $sc['about_page_mission_text'] ?? '' }}</p>
        </div>
        <div class="ab-mv-card">
            <div class="ab-mv-icon vision"><i class="bi bi-eye-fill"></i></div>
            <h3>{{ $sc['about_page_vision_heading'] ?? 'Our Vision' }}</h3>
            <p>{{ $sc['about_page_vision_text'] ?? '' }}</p>
        </div>
    </div>
    ...
    <div class="ab-values-grid">
        <div class="ab-val-card">
            ...
            <div class="ab-val-title">{{ $sc['about_page_val1_title'] ?? 'Speed' }}</div>
            <div class="ab-val-desc">{{ $sc['about_page_val1_desc'] ?? '' }}</div>
        </div>
        <div class="ab-val-card">
            <div class="ab-val-icon"><i class="bi bi-shield-lock-fill"></i></div>
            <div class="ab-val-title">{{ $sc['about_page_val2_title'] ?? 'Security' }}</div>
            <div class="ab-val-desc">{{ $sc['about_page_val2_desc'] ?? '' }}</div>
        </div>
        <div class="ab-val-card">
            <div class="ab-val-icon"><i class="bi bi-eye-fill"></i></div>
            <div class="ab-val-title">{{ $sc['about_page_val3_title'] ?? 'Transparency' }}</div>
            <div class="ab-val-desc">{{ $sc['about_page_val3_desc'] ?? '' }}</div>
        </div>
        <div class="ab-val-card">
            <div class="ab-val-icon"><i class="bi bi-headset"></i></div>
            <div class="ab-val-title">{{ $sc['about_page_val4_title'] ?? 'Support' }}</div>
            <div class="ab-val-desc">{{ $sc['about_page_val4_desc'] ?? '' }}</div>
        </div>
    </div>

    {{-- CTA --}}
    <div class="ab-cta mt-4">
        <h3>{{ $sc['about_page_cta_heading'] ?? 'Ready to Start Trading?' }}</h3>
        <p>{{ $sc['about_page_cta_text'] ?? 'Join thousands of Nigerians who trust KayXchange for their crypto needs.' }}</p>
