...
<title>Verify PIN &mdash; {{ config('app.name') }}</title>
...
<div class="pin-shell">
    ...
    <a href="{{ route('dashboard') }}" class="pin-brand">
        <div class="pin-brand-icon">&#x2B22;</div>
        {{ config('app.name') }}
    </a>

    <div class="pin-card">
        @php $initials = strtoupper(mb_substr($user->name, 0, 1)); @endphp
        ...
        @error('pin')
        <div class="pv-err">{{ $message }}</div>
        @else
        <div class="pv-err" id="pvErr"></div>
        @enderror

        <form method="POST" action="{{ route('pin.verify') }}" id="pvForm">
            @csrf
        ...
        <a href="{{ route('pin.change') }}" class="pv-forgot">Forgot PIN? Change it here</a>
    </div>

    <a href="{{ url()->previous() !== url('/verify-pin') ? url()->previous() : route('dashboard') }}" class="pv-back" style="margin-top:1.25rem">
        &#x2190; Go back
    </a>
