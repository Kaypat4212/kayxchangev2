...
<div class="container my-5">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('rates.index') }}" class="btn-back animate__animated animate__fadeIn">Back to Rates</a>
    </div>
    ...
    <div class="row g-4">
        @forelse($rates as $rate)
            <div class="col-md-4 col-sm-6">
                <div class="card-rate animate__animated animate__fadeInUp" style="animation-delay: {{ $loop->index * 0.2 }}s">
                    <h4 class="text-lg font-semibold mb-3">{{ $rate->coin }}</h4>
                    <div class="mb-2">
                        <span class="rate-label">Buy Rate:</span>
                        <span class="rate-value">₦{{ number_format($rate->buy_rate, 2) }}</span>
                    </div>
                    <div>
                        <span class="rate-label">Sell Rate:</span>
                        <span class="rate-value">₦{{ number_format($rate->sell_rate, 2) }}</span>
                    </div>
                </div>
            </div>
        @empty
