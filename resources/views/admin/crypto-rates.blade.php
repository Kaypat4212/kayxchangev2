...
<div class="container-fluid py-3 px-3 px-md-4">
    ...
    <div class="kx-ticker-wrap">
        ...
        <div class="kx-ticker-track">
            <div class="kx-ticker-scroller" id="tickerScroller">
                ...
                @php $chg = round($lr['change_24h'] ?? 0, 2); @endphp
                <div class="kx-tick-item">
                    <img src="https://assets.coincap.io/assets/icons/{{ strtolower($lr['symbol']) }}@2x.png"
                         style="width:18px;height:18px;border-radius:50%;object-fit:cover"
                         onerror="this.style.display='none'"
                         alt="{{ $lr['symbol'] }}">
                    <span class="kx-tick-sym">{{ $lr['symbol'] }}</span>
                    <span class="kx-tick-price">${{ number_format($lr['price_usd'], $lr['price_usd'] >= 1 ? 2 : 6) }}</span>
                    @if($chg >= 0)
                        <span class="kx-tick-up">&#9650; {{ $chg }}%</span>
                    @else
                        <span class="kx-tick-down">&#9660; {{ abs($chg) }}%</span>
                    @endif
                </div>
                @endforeach
    ...
    <div class="kx-stat-row">
        <div class="kx-stat">
            ...
            <div>
                ...
                <div class="kx-stat-value">{{ $totalCoins }}</div>
        ...
        <div class="kx-stat">
            ...
            <div>
                ...
                <div class="kx-stat-value">{{ $liveCount }}</div>
        ...
        <div class="kx-stat">
            ...
            <div>
                ...
                <div class="kx-stat-value" style="font-size:1rem">{{ $lastUpdated ? \Carbon\Carbon::parse($lastUpdated)->diffForHumans() : '—' }}</div>
                <div class="kx-stat-sub">{{ $lastUpdated ? \Carbon\Carbon::parse($lastUpdated)->format('d M H:i') : '' }}</div>
            </div>
        </div>
        <div class="kx-stat">
            ...
            <div>
                ...
                <div class="kx-stat-value">{{ number_format($avgSpread, 1) }}%</div>
    ...
    {{-- ── BULK UPDATE TOOLBAR ── --}}
    <form method="POST" action="{{ route('admin.crypto-rates.bulk-update') }}" id="bulkForm">
        @csrf
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            $spread   = $rate->sell_rate > 0
                                        ...
                                        : 0;
                        @endphp
                        <tr data-id="{{ $rate->id }}" data-usd="{{ $usdPrice ?? 0 }}">
                            <td>
                                <input type="checkbox" name="selected_rates[]" value="{{ $rate->id }}"
                                       class="rate-checkbox" onchange="updateSelectedCount()">
                            </td>
                            <td>
                                <div class="kx-coin-cell">
                                    <img src="https://assets.coincap.io/assets/icons/{{ strtolower($rate->coin) }}@2x.png"
                                         class="kx-coin-logo"
                                         onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"
                                         alt="{{ $rate->coin }}">
                                    <div class="kx-coin-logo-fallback" style="display:none">{{ strtoupper(substr($rate->coin, 0, 3)) }}</div>
                                    <div>
                                        <div class="kx-coin-name">{{ $rate->coin }}</div>
                                        @if($live)
                                        <div class="kx-coin-sym">{{ $live['name'] }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td>
                                @if($usdPrice !== null)
                                    <span class="kx-usd-price">${{ number_format($usdPrice, $usdPrice >= 1 ? 2 : 6) }}</span>
                                @else
                            ...
                            <td>
                                @if($change !== null)
                                    @if($change >= 0)
                                        <span class="kx-change-up">&#9650; {{ $change }}%</span>
                                    @else
                                        <span class="kx-change-down">&#9660; {{ abs($change) }}%</span>
                                    @endif
                                @else
                            ...
                            <td>
                                <div class="kx-input-group">
                                    ...
                                    <input type="number" name="rates[{{ $rate->id }}][buy_rate]"
                                           class="kx-input buy-input" id="buy_{{ $rate->id }}"
                                           value="{{ $rate->buy_rate }}" step="0.01" min="0"
                                           onchange="recalcSpread({{ $rate->id }})">
                                </div>
                            </td>
                            <td>
                                <div class="kx-input-group">
                                    <span class="kx-input-prefix">₦</span>
                                    <input type="number" name="rates[{{ $rate->id }}][sell_rate]"
                                           class="kx-input sell-input" id="sell_{{ $rate->id }}"
                                           value="{{ $rate->sell_rate }}" step="0.01" min="0"
                                           onchange="recalcSpread({{ $rate->id }})">
                                </div>
                            </td>
                            <td>
                                <span class="kx-spread" id="spread_{{ $rate->id }}">{{ $spread }}%</span>
                            </td>
                            <td style="font-size:.75rem;color:var(--kx-muted)">
                                {{ $rate->updated_at ? \Carbon\Carbon::parse($rate->updated_at)->diffForHumans() : '—' }}
                            </td>
                            <td>
                                <div class="d-flex gap-1 flex-wrap">
                                    <button type="button" class="btn btn-kx-edit"
                                            onclick="saveIndividual({{ $rate->id }}, '{{ $rate->coin }}')">
                                        <i class="bi bi-save"></i>
                                    </button>
                                    <button type="button" class="btn btn-kx-danger"
                                            onclick="confirmDelete({{ $rate->id }}, '{{ $rate->coin }}')">
...
<div class="modal fade" id="addCoinModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.crypto-rates.add') }}">
                @csrf
                <div class="modal-body" style="padding:1.25rem">
                    <div class="mb-3">
                        ...
                        <input type="text" name="coin" class="form-control kx-input" style="width:100%"
                               placeholder="e.g. BTC, ETH, SOL" maxlength="10" required value="{{ old('coin') }}">
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <input type="number" name="buy_rate" class="form-control kx-input" style="width:100%"
                                   placeholder="0.00" step="0.01" min="0" required value="{{ old('buy_rate') }}">
                        </div>
                        <div class="col-6">
                            ...
                            <input type="number" name="sell_rate" class="form-control kx-input" style="width:100%"
                                   placeholder="0.00" step="0.01" min="0" required value="{{ old('sell_rate') }}">
                        </div>
                    </div>
                </div>
...
{{-- ── INDIVIDUAL SAVE FORMS (hidden) ── --}}
@foreach($rates as $rate)
<form id="saveForm_{{ $rate->id }}" method="POST"
      action="{{ route('admin.crypto-rates.update', $rate->id) }}" style="display:none">
    @csrf
    <input type="hidden" name="buy_rate" id="hBuy_{{ $rate->id }}">
    <input type="hidden" name="sell_rate" id="hSell_{{ $rate->id }}">
</form>
@endforeach
...
function refreshTicker() {
    fetch('{{ route("admin.crypto-rates.live-rates") }}')
        .then(r => r.json())
        .then(d => {
