...
<div class="container-fluid px-3 px-md-4 py-4">
    ...
    <div class="kx-page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        ...
        <a href="{{ route('admin.email-templates') }}" class="btn-outline d-inline-flex align-items-center gap-1">
    ...
    <form method="POST" action="{{ route('admin.email-templates.update', $template->key) }}">
        @csrf
        @method('PUT')

        <div class="kx-card">
            <div class="badge-key">{{ $template->key }}</div>
            @if($template->description)
            <p style="color:var(--kx-muted);font-size:.82rem;margin-bottom:1.25rem;">{{ $template->description }}</p>
            @endif

            <div class="mb-3">
                <label class="kx-label">Subject Line</label>
                <input id="email-subject-input" type="text" name="subject" class="kx-input @error('subject') border-danger @enderror"
                       value="{{ old('subject', $template->subject) }}">
                @error('subject')<div class="text-danger mt-1" style="font-size:.8rem;">{{ $message }}</div>@enderror
            ...
            <div class="mb-3">
                ...
                <div class="mb-2">
                    ...
                    <span class="token-pill" onclick="insertToken('&#123;&#123;{{ $tok }}&#125;&#125;')">&#123;&#123;{{ $tok }}&#125;&#125;</span>
                    @endforeach
                </div>

                <textarea id="bodyEditor" name="body" class="kx-input @error('body') border-danger @enderror">{{ old('body', $template->body) }}</textarea>
            ...
            <div class="d-flex gap-2 mt-1">
                ...
                <a href="{{ route('admin.email-templates') }}" class="btn-outline">Cancel</a>
            </div>
        </div>
    </form>
...
async function aiOptimizeSubject(){
    ...
    const templateKey    = '{{ $template->key ?? $template->name ?? "email template" }}';
    ...
    try {
        const res  = await fetch('{{ route("ai.email-subject") }}', {
            method: 'POST',
