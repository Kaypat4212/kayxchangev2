...
<div class="container-fluid py-4 px-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('admin.telegram.index') }}" class="kx-btn kx-btn-muted">← Bot Settings</a>
    </div>

    {{-- Stats --}}
    <div class="kx-stat-grid">
        <div class="kx-stat">
            <div class="kx-stat-val">{{ number_format($totalMessages) }}</div>
            <div class="kx-stat-label">Total Messages</div>
        </div>
        <div class="kx-stat">
            <div class="kx-stat-val">{{ number_format($uniqueSenders) }}</div>
            <div class="kx-stat-label">Unique Users</div>
        </div>
        <div class="kx-stat">
            <div class="kx-stat-val">{{ number_format($todayMessages) }}</div>
        ...
        <div class="kx-panel-body">
            <form method="GET" class="d-flex gap-3 flex-wrap align-items-end">
                <div>
                    ...
                    <input name="search" value="{{ request('search') }}" placeholder="Name, username, chat ID, message…"
                ...
                <div>
                    ...
                    <select name="type" class="kx-input">
                        ...
                        <option value="command"  {{ request('type')=='command'  ? 'selected':'' }}>⌨️ Commands</option>
                        ...
                        <option value="sticker"  {{ request('type')=='sticker'  ? 'selected':'' }}>🎭 Stickers</option>
                ...
                @if(request()->hasAny(['search','type']))
                    <a href="{{ route('admin.telegram.messages') }}" class="kx-btn kx-btn-muted">✕ Clear</a>
                @endif
            </form>
        </div>
    </div>
    ...
    <div class="kx-panel">
        <div class="kx-panel-header">
            <h5>📨 Messages <span class="kx-badge kx-badge-blue ms-2">{{ $messages->total() }} results</span></h5>
        </div>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar">{{ strtoupper(substr($msg->first_name ?: ($msg->username ?: '?'), 0, 1)) }}</div>
                                    <div>
                                        <div class="fw-semibold" style="font-size:0.85rem">
                                            {{ $msg->first_name ?: ('Chat #'.$msg->chat_id) }}
                                        </div>
                                        @if($msg->username)
                                            <div class="msg-meta">{{ '@'.$msg->username }}</div>
                                        @endif
                                        <div class="msg-meta">ID: {{ $msg->chat_id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                @php $mt = $msg->message_type; @endphp
                                @if($mt === 'photo' && $msg->file_id)
                                    <img src="{{ route('admin.telegram.file', $msg->file_id) }}"
                                         class="msg-thumb"
                                         loading="lazy"
                                         alt="photo"
                                         onclick="openImgPreview(this.src)">
                                    @if($msg->message_text && $msg->message_text !== '[photo]')
                                        <div class="msg-meta mt-1" style="max-width:260px; white-space:normal">{{ $msg->message_text }}</div>
                                    @endif
                                @elseif($mt === 'sticker')
                                    <span class="msg-sticker" title="Sticker">{{ $msg->message_text }}</span>
                                    @if($msg->file_name)
                                        <div class="msg-meta">{{ $msg->file_name }}</div>
                                    @endif
                                @elseif(in_array($mt, ['document','video','audio','voice']))
                                    <div class="msg-file">
                                        ...
                                        <div>
                                            <div class="msg-file-name">{{ $msg->file_name ?: $msg->message_text ?: '—' }}</div>
                                            @if($msg->file_id)
                                                <a href="{{ route('admin.telegram.file', $msg->file_id) }}"
                                                   target="_blank"
                                ...
                                @else
                                    <div class="msg-emoji-text" title="{{ $msg->message_text }}">{{ $msg->message_text ?? '—' }}</div>
                                @endif
                            </td>
                            <td>
                                @php
                                ...
                                @endphp
                                <span class="kx-badge {{ $t['class'] }}">{{ $t['icon'] }} {{ $t['label'] }}</span>
                            </td>
                            <td>
                                @if($msg->state_at_time)
                                    <span class="kx-badge kx-badge-amber" style="font-size:0.7rem">{{ $msg->state_at_time }}</span>
                                @else
                            ...
                            <td>
                                @if($msg->user)
                                    <div style="font-size:0.82rem">
                                        <div class="fw-semibold">{{ $msg->user->name }}</div>
                                        <div class="msg-meta">{{ $msg->user->email }}</div>
                                    </div>
                            ...
                            <td>
                                ...
                                <div class="msg-meta">{{ $msg->created_at->format('H:i:s') }}</div>
                            </td>
                            <td>
                                <button
                                    onclick="openReply({{ $msg->chat_id }}, '{{ addslashes($msg->first_name ?? ('@'.($msg->username ?? 'Chat #'.$msg->chat_id))) }}', @json($msg->message_text ?? ''))"
...
<div id="replyModal" style="display:none; position:fixed; inset:0; z-index:9999;
    ...
    <div style="background:var(--kx-card); border:1px solid var(--kx-border); border-radius:14px;
        ...
        <form id="replyForm" method="POST" action="{{ route('admin.telegram.reply') }}">
            @csrf
...
async function aiSuggestReply() {
    ...
    try {
        const res = await fetch('{{ route("admin.telegram.ai-suggest") }}', {
            method: 'POST',
            headers: {
