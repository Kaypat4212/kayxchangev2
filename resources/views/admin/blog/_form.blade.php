{{-- ╔══════════════════════════════════════════════════════════════╗
     ║  Modern Blog Editor  –  shared by create.blade & edit.blade  ║
     ╚══════════════════════════════════════════════════════════════╝ --}}
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        <div class="editor-card mb-3">
            ...
            @error('title')<div class="invalid-feedback ps-2">{{ $message }}</div>@enderror
            ...
            <div class="slug-preview px-3 pb-2 pt-1">
                ...
                <span id="slug-preview-text" class="text-muted font-monospace" style="font-size:.78rem;">
                    {{ old('title', isset($post) ? $post->slug : '') ?: 'auto-generated' }}
                </span>
            </div>
        </div>
        ...
        <div class="editor-card mb-3 p-0 overflow-hidden">
            ...
            <textarea name="content" id="content-input" class="d-none @error('content') is-invalid @enderror">{{ old('content', isset($post) ? $post->content : '') }}</textarea>
            @error('content')<div class="invalid-feedback px-3 pb-2 d-block">{{ $message }}</div>@enderror
        </div>
        ...
        <div class="editor-card mb-3">
            ...
            <textarea name="excerpt" id="excerpt-input" rows="3"
                      class="form-control border-0 shadow-none px-0 @error('excerpt') is-invalid @enderror"
                      placeholder="Write a short, compelling summary of this post (max 500 chars)…"
                      maxlength="500">{{ old('excerpt', isset($post) ? $post->excerpt : '') }}</textarea>
            <div class="d-flex justify-content-end mt-1">
                <span id="excerpt-counter" class="text-muted" style="font-size:.75rem;">
                    <span id="excerpt-chars">{{ strlen(old('excerpt', isset($post) ? $post->excerpt : '')) }}</span>/500
                </span>
            </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="editor-card mb-3">
            ...
            <div class="d-flex align-items-center justify-content-between bg-light rounded-3 p-3 mb-3">
                <div>
                    ...
                    <small id="publish-status-label" class="text-muted" style="font-size:.78rem;">
                        {{ old('is_published', isset($post) ? $post->is_published : false) ? 'Published — visible to all' : 'Draft — not visible' }}
                    </small>
                </div>
                <div class="form-check form-switch m-0">
                    <input class="form-check-input" type="checkbox" role="switch"
                           ...
                           @checked(old('is_published', isset($post) ? $post->is_published : false))>
                </div>
            </div>
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                <span class="badge bg-light text-dark border" style="font-size:.73rem;">
                    <i class="bi bi-calendar-check me-1"></i>{{ $post->published_at->format('M d, Y') }}
                </span>
                @endif
            </div>
        </div>
        ...
        <div class="editor-card mb-3">
            ...
            <div id="cover-preview-wrapper" class="{{ old('cover_image', isset($post) ? $post->cover_image : '') ? '' : 'd-none' }} mb-2">
                <img id="cover-preview-img"
                     src="{{ old('cover_image', isset($post) ? $post->cover_image : '') }}"
                     alt="Cover preview"
            ...
            <input type="url" name="cover_image" id="cover-image-input"
                   ...
                   value="{{ old('cover_image', isset($post) ? $post->cover_image : '') }}">
        ...
        <div class="editor-card mb-3">
            ...
            <input type="text" name="category" id="category-input"
                   ...
                   value="{{ old('category', isset($post) ? $post->category : '') }}"
            ...
            <div class="d-flex flex-wrap gap-1 mt-2">
                @foreach(["Beginner's Guide","Market Insights","Trading Tips","Crypto News","Security","How It Works","General"] as $cat)
                <button type="button" class="category-chip" data-value="{{ $cat }}">{{ $cat }}</button>
                @endforeach
            </div>
        </div>
        ...
        <div class="editor-card">
            ...
            <div class="seo-preview rounded-3 p-3">
                ...
                <div class="seo-title" id="seo-title">{{ isset($post) && $post->title ? $post->title : 'Post title will appear here' }}</div>
                <div class="seo-desc" id="seo-desc">{{ isset($post) && $post->excerpt ? $post->excerpt : 'Excerpt will appear here as the meta description.' }}</div>
            </div>
        </div>
