...
<div class="container-fluid px-3 py-4">
    ...
    @if(session('success'))
        <div class="kx-alert success"><i class="bi bi-check-circle me-2"></i>{{ session('success') }}</div>
    @endif
    @if(session('error'))
        <div class="kx-alert error"><i class="bi bi-x-circle me-2"></i>{{ session('error') }}</div>
    @endif
    ...
    <div class="row g-3">
        ...
        <div class="col-lg-5">
            ...
            <div class="kx-card">
                ...
                <form method="POST" action="{{ route('admin.email-settings.update') }}" id="smtp-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="mail_from_name" class="kx-input" value="{{ $settings->mail_from_name }}" placeholder="KayXchange">
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-3">
                        <div class="kx-toggle">
                            <input type="checkbox" id="toggle_welcome" name="welcome_email_enabled" value="1"
                                {{ $settings->welcome_email_enabled ? 'checked' : '' }}>
                        ...
                        <div class="kx-toggle">
                            <input type="checkbox" id="toggle_login_success" name="login_success_email_enabled" value="1"
                                {{ $settings->login_success_email_enabled ? 'checked' : '' }}>
                        ...
                        <div class="kx-toggle">
                            <input type="checkbox" id="toggle_login_failed" name="login_failed_email_enabled" value="1"
                                {{ $settings->login_failed_email_enabled ? 'checked' : '' }}>
        ...
        {{-- ===== RIGHT COL: Login Logs ===== --}}
        <div class="col-lg-7">

            {{-- Stats --}}
            <div class="kx-stats">
                <div class="kx-stat">
                    <div class="s-val">{{ $totalAttempts }}</div>
                    <div class="s-lbl">Total Attempts</div>
                </div>
                <div class="kx-stat">
                    <div class="s-val">{{ $totalSuccess }}</div>
                    <div class="s-lbl">Successful</div>
                </div>
                <div class="kx-stat">
                    <div class="s-val danger">{{ $totalFailed }}</div>
            ...
            <div class="kx-card">
                ...
                {{-- Filter --}}
                <form method="GET" action="{{ route('admin.email-settings.index') }}" class="filter-bar mb-3">
                    <input type="text" name="search" class="kx-input" placeholder="Search email..." value="{{ request('search') }}">
                    ...
                    <a href="{{ route('admin.email-settings.index') }}" class="btn-kx-outline" style="padding:.6rem 1rem;font-size:.82rem;text-decoration:none;">
                        <i class="bi bi-x"></i>
                    </a>
                    @endif
                </form>
                ...
                <div class="table-responsive">
                    <table class="kx-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <div style="font-weight:600;color:var(--kx-text);">{{ $log->email }}</div>
                                    @if($log->user)
                                    <div style="font-size:.72rem;color:var(--kx-muted);">{{ $log->user->name }}</div>
                                    @endif
                                </td>
                                ...
                                <td class="text-end" style="color:var(--kx-muted);white-space:nowrap;font-size:.75rem;">
                                    ...
                                    <span style="font-size:.7rem;">{{ $log->created_at->format('g:i A') }}</span>
                                </td>
                            </tr>
                            @empty
                            <tr>
