...
<div class="st-wrap">
    ...
    <div class="st-profile">
        <div class="st-avatar">{{ substr(auth()->user()->name ?? 'U', 0, 1) }}</div>
        <div>
            <div class="st-profile-name">{{ auth()->user()->name }}</div>
            <div class="st-profile-email">{{ auth()->user()->email }}</div>
    ...
    <a href="{{ route('edit-bank') }}" class="st-row">
        ...
        <div class="st-row-body">
            ...
            <div class="st-row-sub">
                @if(auth()->user()->account_number)
                    {{ auth()->user()->account_name }} &mdash; {{ auth()->user()->account_number }}
                @else
    ...
    <a href="{{ route('change.password.form') }}" class="st-row">
    ...
    <a href="{{ route('pin.change') }}" class="st-row">
        <div class="st-row-icon" style="background:rgba(0,204,0,.1);color:#00cc00;"><i class="bi bi-grid-3x3-gap-fill"></i></div>
        <div class="st-row-body">
            <div class="st-row-title">{{ auth()->user()->transaction_pin ? 'Change PIN' : 'Set Up PIN' }}</div>
            <div class="st-row-sub">{{ auth()->user()->transaction_pin ? '4-digit transaction security PIN' : 'Protect withdrawals with a PIN' }}</div>
        </div>
        <i class="bi bi-chevron-right st-row-arrow"></i>
    </a>
    ...
    <a href="{{ route('settings.telegram') }}" class="st-row">
    ...
    <a href="{{ route('profile.edit') }}#delete-account" class="st-danger-zone">
