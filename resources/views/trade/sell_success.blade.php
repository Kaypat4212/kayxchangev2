    ...
    <div class="w-full max-w-md bg-gray-800 rounded-2xl shadow-2xl p-6 sm:p-8 transform transition-all hover:shadow-blue-500/20 duration-300">
        ...
        <div class="space-y-4">
            ...
            <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold text-white">{{ $trade->transaction_ref }}</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold text-white capitalize">{{ str_replace('_', ' ', $trade->payment_method) }}</span>
            </div>
            @if($trade->payment_method !== 'wallet_balance')
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-gray-300">Bank Name:</span>
                    <span class="text-sm font-semibold text-white">{{ $trade->bank_name }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-gray-300">Account Number:</span>
                    <span class="text-sm font-semibold text-white">{{ $trade->account_number }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-gray-300">Account Holder:</span>
                    <span class="text-sm font-semibold text-white">{{ $trade->account_name }}</span>
                </div>
            @endif
            <div class="flex justify-between">
        ...
        <div class="mt-6">
            <a href="{{ route('sell.success', ['trade_id' => $trade->id]) }}" class="w-full bg-gradient-to-r from-blue-600 to-teal-500 p-3 rounded-lg font-semibold text-white shadow-md hover:from-blue-700 hover:to-teal-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200 text-center inline-block">
                Confirm and Proceed
            </a>
        </div>
    </div>
