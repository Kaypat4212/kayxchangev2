...
<div class="col-xl-8 col-lg-9">
    ...
    <div class="bls-breadcrumb">
        <a href="{{ url('/blog') }}"><i class="bi bi-journal-richtext me-1"></i>Blog</a>
        <span class="mx-1">/</span>
        <span>{{ $post->category }}</span>
    </div>

    @if($post->cover_image)
        <img src="{{ asset('storage/'.$post->cover_image) }}" alt="{{ $post->title }}" class="bls-cover">
    @else
        <div class="bls-cover-placeholder"><i class="bi bi-journal-text"></i></div>
    @endif

    <span class="bls-cat">{{ $post->category }}</span>
    <h1 class="bls-title">{{ $post->title }}</h1>

    <div class="bls-meta">
        <span><i class="bi bi-calendar3"></i>{{ $post->published_at?->format('F d, Y') }}</span>
        <span><i class="bi bi-clock"></i>{{ $post->readingTime() }} min read</span>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        <a href="{{ url('/blog') }}" class="btn btn-sm btn-outline-secondary" style="border-color:rgba(255,255,255,.15);color:rgba(255,255,255,.6);border-radius:8px;">
            <i class="bi bi-arrow-left me-1"></i>Back to Blog
        </a>
        <a href="{{ url('/blog?category='.urlencode($post->category)) }}"
           class="btn btn-sm" style="background:rgba(0,204,0,.1);border:1px solid rgba(0,204,0,.25);color:#00cc00;border-radius:8px;">
            <i class="bi bi-tag me-1"></i>More in {{ $post->category }}
        </a>
    </div>
    ...
    <div class="bls-related-grid">
        ...
        <a href="{{ url('/blog/'.$rel->slug) }}" class="bls-rel-card">
            <div class="bls-rel-cat">{{ $rel->category }}</div>
            <div class="bls-rel-title">{{ $rel->title }}</div>
            <div class="bls-rel-date"><i class="bi bi-calendar3 me-1"></i>{{ $rel->published_at?->format('M d, Y') }}</div>
        </a>
        @endforeach
    </div>
    @endif
