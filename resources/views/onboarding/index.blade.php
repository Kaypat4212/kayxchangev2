...
<div class="ob-shell">
    ...
    <div class="ob-card">
        ...
        <div class="ob-step active" id="step0">
            ...
            <h1 class="ob-welcome-title">Welcome,<br>{{ explode(' ', Auth::user()->name)[0] }}!</h1>
...
window.savePinToServer = async function() {
    ...
    try {
        const res = await fetch('{{ route("onboard.pin") }}', {
            method: 'POST',
            headers: {
...
window.saveBank = async function() {
    ...
    try {
        const res = await fetch('{{ route("onboard.bank") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
window.finish = async function() {
    ...
    try {
        const res = await fetch('{{ route("onboard.complete") }}', {
            method: 'POST',
