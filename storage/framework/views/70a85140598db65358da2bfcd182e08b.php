...
<div class="container-fluid py-3 px-3 px-md-4">
    ...
    <div class="kx-stat-row">
        <div class="kx-stat"><div class="kx-stat-icon icon-blue"><i class="bi bi-graph-up"></i></div>
            <div><div class="kx-stat-label">Total Tx</div><div class="kx-stat-value"><?php echo e(number_format($statistics['total_transactions'])); ?></div></div></div>
        <div class="kx-stat"><div class="kx-stat-icon icon-yellow"><i class="bi bi-clock-history"></i></div>
            <div><div class="kx-stat-label">Pending</div><div class="kx-stat-value"><?php echo e(number_format($statistics['pending_transactions'])); ?></div></div></div>
        <div class="kx-stat"><div class="kx-stat-icon icon-green"><i class="bi bi-check-circle-fill"></i></div>
            <div><div class="kx-stat-label">Completed</div><div class="kx-stat-value"><?php echo e(number_format($statistics['completed_transactions'])); ?></div></div></div>
        <div class="kx-stat"><div class="kx-stat-icon icon-purple"><i class="bi bi-cash-coin"></i></div>
            <div><div class="kx-stat-label">Volume</div><div class="kx-stat-value" style="font-size:1rem">₦<?php echo e(number_format($statistics['total_volume'],0)); ?></div></div></div>
    </div>
    ...
    <div class="kx-panel">
        ...
        <div class="nav-tabs-kx">
            <button class="nav-link active" onclick="switchTab('buy',this)">
                ...
                <?php $pendingBuy = $buyTrades->where('status','pending')->count(); ?>
                <?php if($pendingBuy > 0): ?><span class="kx-badge kx-badge-yellow ms-1" style="font-size:.65rem"><?php echo e($pendingBuy); ?></span><?php endif; ?>
            </button>
            <button class="nav-link" onclick="switchTab('sell',this)">
                <i class="bi bi-arrow-up-circle me-1"></i>Sell Trades
                <?php $pendingSell = $sellTrades->where('status','pending')->count(); ?>
                <?php if($pendingSell > 0): ?><span class="kx-badge kx-badge-yellow ms-1" style="font-size:.65rem"><?php echo e($pendingSell); ?></span><?php endif; ?>
            </button>
            <button class="nav-link" onclick="switchTab('dep',this)"><i class="bi bi-bank me-1"></i>Deposits (<?php echo e($deposits->count()); ?>)</button>
            <button class="nav-link" onclick="switchTab('wd',this)"><i class="bi bi-arrow-up-right-circle me-1"></i>Withdrawals (<?php echo e($withdrawals->count()); ?>)</button>
        </div>
        ...
        <div id="tab-buy">
            ...
            <table class="kx-table" id="buy-table">
                ...
                <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $buyTrades; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                    $s = $t->status;
                    ...
                    $user = $t->user;
                ?>
                <tr data-status="<?php echo e($statusClass); ?>" data-created="<?php echo e($t->created_at?->toIso8601String()); ?>" data-kind="buy">
                    <td style="color:var(--kx-muted);white-space:nowrap">#<?php echo e($t->id); ?></td>
                    <td>
                        <div style="font-weight:600;font-size:.84rem"><?php echo e($user->name ?? $t->name ?? 'N/A'); ?></div>
                        <div style="font-size:.72rem;color:var(--kx-muted)"><?php echo e($user->email ?? '—'); ?></div>
                    </td>
                    <td><span class="kx-badge kx-badge-green" style="font-family:monospace"><?php echo e(strtoupper($t->coin ?? '—')); ?></span></td>
                    <td style="color:var(--kx-blue);font-size:.82rem">
                        $<?php echo e(number_format($t->usd_amount ?? 0, 2)); ?>

                        <div class="kx-crypto-sub" data-coin="<?php echo e(strtoupper($t->coin ?? '')); ?>" data-usd="<?php echo e($t->usd_amount ?? 0); ?>" style="font-size:.68rem;color:#f7931a;margin-top:.1rem"></div>
                    </td>
                    <td style="font-weight:700">₦<?php echo e(number_format($t->naira_amount ?? 0, 2)); ?></td>
                    <td style="font-size:.74rem;color:var(--kx-muted);white-space:nowrap"><?php echo e($t->payment_method ?? 'Bank Transfer'); ?></td>
                    <td>
                        <?php if($t->wallet_address): ?>
                            <span style="font-size:.72rem;font-family:monospace;color:var(--kx-muted);max-width:145px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="<?php echo e($t->wallet_address); ?>"><?php echo e($t->wallet_address); ?></span>
                        <?php else: ?>
                    ...
                    <td>
                        <?php if($t->payment_proof): ?>
                            <a href="<?php echo e(asset('storage/'.$t->payment_proof)); ?>" target="_blank" title="View proof">
                                <img src="<?php echo e(asset('storage/'.$t->payment_proof)); ?>" class="proof-thumb" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex'">
                    ...
                    <td>
                        <?php if(in_array($s,['completed','successful','approved'])): ?>
                            <span class="kx-badge kx-badge-green"><i class="bi bi-check me-1"></i><?php echo e(ucfirst($s)); ?></span>
                        <?php elseif($s === 'pending'): ?>
                            <span class="kx-badge kx-badge-yellow"><i class="bi bi-clock me-1"></i>Pending</span>
                        <?php elseif(in_array($s,['failed','cancelled','rejected'])): ?>
                            <span class="kx-badge kx-badge-red"><?php echo e(ucfirst($s)); ?></span>
                        <?php else: ?>
                            <span class="kx-badge kx-badge-gray"><?php echo e(ucfirst($s)); ?></span>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:.74rem;color:var(--kx-muted);white-space:nowrap">
                        <div><?php echo e($t->created_at?->format('d M Y H:i') ?? '—'); ?></div>
                        <?php if($s === 'pending'): ?>
                        <span class="kx-sla-badge kx-sla-ok js-sla-badge" data-created="<?php echo e($t->created_at?->toIso8601String()); ?>" data-kind="buy"></span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <div style="display:flex;gap:.35rem;align-items:center;flex-wrap:wrap">
                            ...
                            <button type="button" class="btn-kx-edit"
                                onclick="openBuyDetail({
                                    id:'<?php echo e($t->id); ?>',
                                    user:'<?php echo e(addslashes($user->name ?? $t->name ?? 'N/A')); ?>',
                                    email:'<?php echo e(addslashes($user->email ?? '—')); ?>',
                                    coin:'<?php echo e(strtoupper($t->coin ?? '—')); ?>',
                                    network:'<?php echo e($t->network ?? '—'); ?>',
                                    wallet:'<?php echo e(addslashes($t->wallet_address ?? '—')); ?>',
                                    usd:'<?php echo e(number_format($t->usd_amount ?? 0,2)); ?>',
                                    naira:'<?php echo e(number_format($t->naira_amount ?? 0,2)); ?>',
                                    proof:'<?php echo e($t->payment_proof ? asset('storage/'.$t->payment_proof) : ''); ?>',
                                    adminProof:'<?php echo e($t->admin_payment_proof ? asset('storage/'.$t->admin_payment_proof) : ''); ?>',
                                    txid:'<?php echo e(addslashes($t->blockchain_txid ?? '—')); ?>',
                                    method:'<?php echo e(addslashes($t->payment_method ?? 'Bank Transfer')); ?>',
                                    status:'<?php echo e($s); ?>'
                                })">
                                <i class="bi bi-eye"></i> View
                            </button>
                            
                            <form id="buy-form-<?php echo e($t->id); ?>" action="<?php echo e(route('admin.buy.updateStatus', $t->id)); ?>" method="POST" style="display:flex;gap:.3rem;align-items:center">
                                <?php echo csrf_field(); ?>
                                <select name="status" id="buy-sel-<?php echo e($t->id); ?>" class="kx-input" style="width:108px;padding:.3rem .5rem!important;font-size:.72rem!important">
                                    <?php $__currentLoopData = ['pending','completed','rejected']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($st); ?>" <?php echo e($s === $st ? 'selected':''); ?>><?php echo e(ucfirst($st)); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                                <button type="button"
                                    class="btn-kx-green"
                                    style="padding:.3rem .6rem;font-size:.72rem"
                                    onclick="handleBuyApprove(<?php echo e($t->id); ?>,'<?php echo e(addslashes($user->name ?? $t->name ?? 'N/A')); ?>','<?php echo e(addslashes($user->bank_name ?? 'N/A')); ?>','<?php echo e(addslashes($user->account_number ?? 'N/A')); ?>','<?php echo e(addslashes($user->account_name ?? 'N/A')); ?>','<?php echo e(addslashes($t->wallet_address ?? 'N/A')); ?>','<?php echo e(strtoupper($t->coin ?? '')); ?>','<?php echo e(number_format($t->naira_amount ?? 0,2)); ?>','<?php echo e(number_format($t->usd_amount ?? 0,2)); ?>','<?php echo e(addslashes($t->blockchain_txid ?? '')); ?>','<?php echo e($t->admin_payment_proof ? asset('storage/'.$t->admin_payment_proof) : ''); ?>')">
        ...
        <div id="tab-sell" style="display:none">
            ...
            <table class="kx-table" id="sell-table">
                ...
                <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $sellTrades; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                    $s = $t->status;
                    $statusClass = in_array($s,['completed','successful','approved']) ? 'completed'
                        : ($s==='pending' ? 'pending'
                    ...
                    $user = $t->user;
                ?>
                <tr data-status="<?php echo e($statusClass); ?>" data-created="<?php echo e($t->created_at?->toIso8601String()); ?>" data-kind="sell">
                    <td style="color:var(--kx-muted);white-space:nowrap">#<?php echo e($t->id); ?></td>
                    <td>
                        <div style="font-weight:600;font-size:.84rem"><?php echo e($user->name ?? $t->name ?? 'N/A'); ?></div>
                        <div style="font-size:.72rem;color:var(--kx-muted)"><?php echo e($user->email ?? '—'); ?></div>
                    </td>
                    <td><span class="kx-badge kx-badge-yellow" style="font-family:monospace"><?php echo e(strtoupper($t->coin ?? '—')); ?></span></td>
                    <td style="font-weight:700">
                        ₦<?php echo e(number_format($t->naira_amount ?? $t->amount ?? 0, 2)); ?>

                        <div style="font-size:.72rem;color:var(--kx-blue);margin-top:.1rem">$<?php echo e(number_format($t->usd_amount ?? 0, 2)); ?></div>
                        <div class="kx-crypto-sub" data-coin="<?php echo e(strtoupper($t->coin ?? '')); ?>" data-usd="<?php echo e($t->usd_amount ?? 0); ?>" style="font-size:.68rem;color:var(--kx-yellow);margin-top:.05rem"></div>
                    </td>
                    <td>
                        
                        <?php if($t->bank_name || $t->account_number): ?>
                        <div style="font-size:.76rem;line-height:1.6">
                            <div style="color:var(--kx-text);font-weight:600"><?php echo e($t->account_name ?? '—'); ?></div>
                            <div style="color:var(--kx-muted)"><?php echo e($t->bank_name ?? '—'); ?></div>
                            <div style="color:var(--kx-blue);font-family:monospace;font-size:.78rem"><?php echo e($t->account_number ?? '—'); ?></div>
                        </div>
                        <?php elseif($t->payment_method): ?>
                        <span class="kx-badge kx-badge-blue"><?php echo e($t->payment_method); ?></span>
                        <?php else: ?>
                        <span style="font-size:.75rem;color:var(--kx-muted)">—</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if($t->wallet_address): ?>
                        <span style="font-size:.72rem;font-family:monospace;color:var(--kx-muted);max-width:90px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="<?php echo e($t->wallet_address); ?>"><?php echo e($t->wallet_address); ?></span>
                        <?php else: ?>
                        <span style="color:var(--kx-muted)">—</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php $proofPath = $t->payment_proof ?? $t->proof; ?>
                        <?php if($proofPath): ?>
                            <a href="<?php echo e(asset('storage/'.$proofPath)); ?>" target="_blank">
                                <img src="<?php echo e(asset('storage/'.$proofPath)); ?>" class="proof-thumb" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex'">
                                <span class="btn-kx-icon" style="display:none"><i class="bi bi-image"></i></span>
                            </a>
                        <?php else: ?>
                        <span style="font-size:.75rem;color:var(--kx-muted)">—</span>
                    ...
                    <td style="font-size:.74rem;color:var(--kx-muted);white-space:nowrap">
                        <div><?php echo e($t->created_at?->format('d M Y H:i') ?? '—'); ?></div>
                        <?php if($s === 'pending'): ?>
                        <span class="kx-sla-badge kx-sla-ok js-sla-badge" data-created="<?php echo e($t->created_at?->toIso8601String()); ?>" data-kind="sell"></span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <div style="display:flex;gap:.35rem;align-items:center;flex-wrap:wrap">
                            ...
                            <button type="button" class="btn-kx-edit"
                                onclick="openSellDetail({
                                    ...
                                    wallet:'<?php echo e(addslashes($t->wallet_address ?? '—')); ?>',
                                    usd:'<?php echo e(number_format($t->usd_amount ?? 0,2)); ?>',
                                    naira:'<?php echo e(number_format($t->naira_amount ?? $t->amount ?? 0,2)); ?>',
                                    bank:'<?php echo e(addslashes($t->bank_name ?? '—')); ?>',
                                    accNum:'<?php echo e(addslashes($t->account_number ?? '—')); ?>',
                                    accName:'<?php echo e(addslashes($t->account_name ?? '—')); ?>',
                                    method:'<?php echo e(addslashes($t->payment_method ?? 'Bank Transfer')); ?>',
                                    proof:'<?php echo e($proofPath ? asset('storage/'.$proofPath) : ''); ?>',
                                    status:'<?php echo e($s); ?>'
                                })">
                                <i class="bi bi-eye"></i> View
                            </button>
                            
                            <form id="sell-form-<?php echo e($t->id); ?>" action="<?php echo e(route('admin.sells.updateStatus', $t->id)); ?>" method="POST" style="display:flex;gap:.3rem;align-items:center">
                                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                <select name="status" id="sell-sel-<?php echo e($t->id); ?>" class="kx-input" style="width:108px;padding:.3rem .5rem!important;font-size:.72rem!important">
                                    <?php $__currentLoopData = ['pending','completed','rejected']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($st); ?>" <?php echo e($s === $st ? 'selected':''); ?>><?php echo e(ucfirst($st)); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                                <button type="button"
                                    class="btn-kx-green"
                                    style="padding:.3rem .6rem;font-size:.72rem"
                                    onclick="handleSellApprove(<?php echo e($t->id); ?>,'<?php echo e(addslashes($user->name ?? $t->name ?? 'N/A')); ?>','<?php echo e(addslashes($t->bank_name ?? '—')); ?>','<?php echo e(addslashes($t->account_number ?? '—')); ?>','<?php echo e(addslashes($t->account_name ?? '—')); ?>','<?php echo e(addslashes($t->wallet_address ?? '—')); ?>','<?php echo e(strtoupper($t->coin ?? '')); ?>','<?php echo e(number_format($t->naira_amount ?? $t->amount ?? 0,2)); ?>','<?php echo e(number_format($t->usd_amount ?? 0,2)); ?>')">
                                    <i class="bi bi-save"></i>
                                </button>
                            </form>
                            <?php if($t->user_id): ?>
                            <button type="button" class="btn-kx-icon" title="AI Fraud Check"
                                onclick="aiSpotSuspicious(<?php echo e($t->user_id); ?>, '<?php echo e(addslashes($user->name ?? 'User')); ?>')"
        ...
        <div id="tab-dep" style="display:none">
            ...
            <table class="kx-table">
                ...
                <tr>
                    <td style="color:var(--kx-muted)">#<?php echo e($d->id); ?></td>
                    <td>
                        <div style="font-weight:600;font-size:.84rem"><?php echo e($d->user->name ?? 'N/A'); ?></div>
                        <div style="font-size:.72rem;color:var(--kx-muted)"><?php echo e($d->user->email ?? '—'); ?></div>
                    </td>
                    <td style="font-weight:700;color:var(--kx-blue)">₦<?php echo e(number_format($d->amount, 2)); ?></td>
                    <td style="font-size:.78rem;color:var(--kx-muted)"><?php echo e($d->payment_method ?? 'Bank Transfer'); ?></td>
                    <td>
                        <?php if(in_array($d->status,['approved','successful','completed'])): ?>
                            <span class="kx-badge kx-badge-green"><i class="bi bi-check me-1"></i>Approved</span>
                        <?php elseif($d->status === 'pending'): ?>
                            <span class="kx-badge kx-badge-yellow"><i class="bi bi-clock me-1"></i>Pending</span>
                        <?php else: ?>
                            <span class="kx-badge kx-badge-red"><?php echo e(ucfirst($d->status)); ?></span>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:.74rem;color:var(--kx-muted)"><?php echo e($d->created_at?->format('d M Y H:i') ?? '—'); ?></td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        ...
        <div id="tab-wd" style="display:none">
            ...
            <table class="kx-table">
                ...
                <tr>
                    <td style="color:var(--kx-muted)">#<?php echo e($w->id); ?></td>
                    <td>
                        <div style="font-weight:600;font-size:.84rem"><?php echo e($w->user->name ?? 'N/A'); ?></div>
                        <div style="font-size:.72rem;color:var(--kx-muted)"><?php echo e($w->user->email ?? '—'); ?></div>
                    </td>
                    <td style="font-weight:700;color:var(--kx-yellow)">₦<?php echo e(number_format($w->amount, 2)); ?></td>
                    <td>
                        <?php $wu = $w->user; ?>
                        <?php if($wu && $wu->bank_name): ?>
                        <div style="font-size:.76rem;line-height:1.6">
                            <div style="font-weight:600;color:var(--kx-text)"><?php echo e($wu->account_name ?? '—'); ?></div>
                            <div style="color:var(--kx-muted)"><?php echo e($wu->bank_name); ?></div>
                            <div style="color:var(--kx-blue);font-family:monospace;font-size:.78rem"><?php echo e($wu->account_number ?? '—'); ?></div>
                        </div>
                        <?php else: ?>
                    ...
                    <td>
                        ...
                        <?php elseif($w->status === 'pending'): ?>
                            <span class="kx-badge kx-badge-yellow"><i class="bi bi-clock me-1"></i>Pending</span>
                        <?php else: ?>
                            <span class="kx-badge kx-badge-red"><?php echo e(ucfirst($w->status)); ?></span>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:.74rem;color:var(--kx-muted)"><?php echo e($w->created_at?->format('d M Y H:i') ?? '—'); ?></td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
// ── Pending SLA badges ─────────────────────────────
const SLA_MINUTES = <?php echo e((int) config('trade_alerts.pending_sla_minutes', 20)); ?>;
const ESCALATE_MINUTES = <?php echo e((int) config('trade_alerts.escalate_after_minutes', 30)); ?>;
...
async function aiTradeSummary(){
    ...
    try {
        const res = await fetch('<?php echo e(route("ai.trade-summary")); ?>', {
            method: 'POST',
...
async function aiSpotSuspicious(userId, userName){
    ...
    modal.show();
    try {
        const res = await fetch('<?php echo e(route("ai.spot-suspicious")); ?>', {
            method: 'POST',
            headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},
...
<?php echo $__env->make('adminnavlayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\kayxchangev2\resources\views\admin\trades.blade.php ENDPATH**/ ?>