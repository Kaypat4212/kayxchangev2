    ...
    <div class="container-xl">
        ...
        <div class="kx-kyc-banner mb-4" id="kx-kyc-banner">
            <div class="d-flex align-items-center gap-3 flex-wrap">
                ...
                <a href="<?php echo e(route('kyc.form')); ?>" class="kx-kyc-btn ms-auto">
        ...
        <div class="kx-db-head">
            <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
                <div>
                    ...
                    <h1 class="kx-db-name"><?php echo e(Auth::user()->name); ?></h1>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <?php else: ?>
                            ...
                            <a href="<?php echo e(route('kyc.form')); ?>" class="btn btn-sm" style="background:rgba(251,191,36,.12);border:1px solid rgba(251,191,36,.32);color:#fbbf24;border-radius:999px;font-size:.72rem;font-weight:600;padding:.22rem .65rem;">
                                <i class="bi bi-arrow-right-circle me-1"></i>Verify Now
                            </a>
                        <?php endif; ?>
                        <span style="font-size:0.72rem;color:rgba(255,255,255,0.26)"><?php echo e(now()->format('l, d M Y')); ?></span>
                    </div>
                </div>
        ...
        <div class="row g-4 mb-4">
            <div class="col-12 col-md-5 col-lg-4">
                <div class="kx-balance-card">
                    ...
                    <div class="kx-balance-amount">₦<?php echo e(number_format(Auth::user()->balance, 2)); ?></div>
                    <?php
                    ...
                    <div class="kx-balance-sub" style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;">
                        ...
                        <?php if($usdtRate > 0): ?>
                        <span class="kx-usd-equiv" title="Based on USDT sell rate ₦<?php echo e(number_format($usdtRate,2)); ?>/USD">
                            ≈ $<?php echo e(number_format($usdEquiv, 2)); ?> USD
                        </span>
                        <?php endif; ?>
                    </div>
                    <div class="d-flex gap-2 mt-4" style="position:relative;z-index:1">
                        <a href="<?php echo e(route('deposits.index')); ?>" class="kx-bal-btn kx-bal-dep">
                            <i class="bi bi-arrow-down-circle-fill"></i>Deposit
                        </a>
                        <a href="<?php echo e(route('withdraw')); ?>" class="kx-bal-btn kx-bal-wd">
                ...
                <div class="row g-3 h-100">
                    <div class="col-6 col-sm-4">
                        <div class="kx-card kx-stat h-100">
                            ...
                            <div class="kx-stat-val"><?php echo e(isset($transactions) ? count($transactions) : '0'); ?></div>
                        </div>
                    </div>
                    <div class="col-6 col-sm-4">
                        <div class="kx-card kx-stat h-100">
                    ...
                    <div class="col-6 col-sm-4">
                        <div class="kx-card kx-stat h-100">
                            ...
                            <div class="kx-stat-val"><?php echo e(isset($referralCount) ? $referralCount : '–'); ?></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="kx-card p-3 mb-4">
            <div class="kx-quick">
                <a href="<?php echo e(Auth::user()->kyc_verified ? route('buy') : route('kyc.form')); ?>" class="kx-qbtn kx-qi-buy">
                    <div class="kx-qicon"><i class="bi bi-cart-fill"></i></div>
                    Buy Crypto
                </a>
                <a href="<?php echo e(route('sell.form')); ?>" class="kx-qbtn kx-qi-sell">
                    <div class="kx-qicon"><i class="bi bi-bag-fill"></i></div>
                    Sell Crypto
                </a>
                <a href="<?php echo e(route('deposits.index')); ?>" class="kx-qbtn kx-qi-dep">
                    <div class="kx-qicon"><i class="bi bi-arrow-down-circle-fill"></i></div>
                    Deposit
                </a>
                <a href="<?php echo e(route('withdraw')); ?>" class="kx-qbtn kx-qi-wd">
                    <div class="kx-qicon"><i class="bi bi-arrow-up-circle-fill"></i></div>
                    Withdraw
                </a>
                <a href="<?php echo e(route('transactions.history')); ?>" class="kx-qbtn kx-qi-hist">
                    <div class="kx-qicon"><i class="bi bi-clock-history"></i></div>
                    History
                </a>
                <a href="<?php echo e(route('referrals')); ?>" class="kx-qbtn kx-qi-ref">
        ...
        <div class="kx-card p-4 mb-4">
            ...
            <div class="kx-tx-scroll">
                <table class="kx-db-tbl">
                    ...
                    <tbody id="transactions-container">
                        <?php if(isset($transactions) && !$transactions->isEmpty()): ?>
                            <?php $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td><?php echo e($transaction['created_at']->toDateString()); ?></td>
                                    <td>
                                        <?php $t = $transaction['type']; ?>
                                        <span class="kx-tx-badge kx-tx-<?php echo e($t === 'buy' ? 'buy' : ($t === 'sell' ? 'sell' : ($t === 'withdrawal' ? 'wd' : ($t === 'deposit' ? 'dep' : 'other')))); ?>"><?php echo e(ucfirst($t)); ?></span>
                                    </td>
                                    <td><?php echo e($t === 'withdrawal' ? ($transaction['bank_account'] ?? 'N/A') : ($transaction['coin'] ?? 'N/A')); ?></td>
                                    <td class="<?php echo e($t === 'buy' ? 'kx-rate-buy' : ($t === 'sell' ? 'kx-rate-sell' : '')); ?>"><?php echo e($transaction['amount_ngn'] ? '₦' . number_format($transaction['amount_ngn'], 2) : 'N/A'); ?></td>
                                    <td><?php echo e($transaction['amount_usd'] ? '$' . number_format($transaction['amount_usd'], 2) : 'N/A'); ?></td>
                                    <td><span class="kx-status kx-status-<?php echo e(strtolower($transaction['status'] ?? 'other')); ?>"><?php echo e($transaction['status'] ?? 'N/A'); ?></span></td>
                                </tr>
            ...
            <?php if(isset($transactions) && $transactions->count() > 3): ?>
                <div class="text-center mt-4">
                    <a href="<?php echo e(route('transactions.history')); ?>" class="kx-btn-more"><i class="bi bi-list-ul"></i>View All Transactions</a>
                </div>
            <?php endif; ?>
        </div>
...
<script>
    ...
    const notifApiUrl = <?php echo json_encode(route('notifications.api'), 15, 512) ?>;
    const notifIndexUrl = <?php echo json_encode(route('notifications.index'), 15, 512) ?>;
    const notifMarkAllUrl = <?php echo json_encode(route('notifications.mark-all-read'), 15, 512) ?>;
    ...
    function fetchRates() {
        ...
        fetch('/crypto-rates', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .catch(() => {
                const fb = <?php echo json_encode($rates, 15, 512) ?>;
    ...
    function fetchPrices() {
        ...
        fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,tether,binancecoin,litecoin&order=market_cap_desc&price_change_percentage=24h', { headers: { 'Accept': 'application/json' } })
            ...
            .catch(() => {
                const fb = <?php echo json_encode(isset($prices) ? $prices : [], 15, 512) ?>;
                if (Object.keys(fb).length) {
...
(function(){
    fetch('<?php echo e(route("ai.dashboard-insight")); ?>', {
        method: 'POST',
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\kayxchangev2\resources\views/dashboard.blade.php ENDPATH**/ ?>