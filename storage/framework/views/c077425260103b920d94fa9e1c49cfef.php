...
<div class="container-fluid py-3 px-3 px-md-4">
    ...
    <div class="kx-stat-row">
        <div class="kx-stat"><div class="kx-stat-icon icon-yellow"><i class="bi bi-hourglass-split"></i></div>
            <div><div class="kx-stat-label">Pending</div><div class="kx-stat-value"><?php echo e($depPending); ?></div></div></div>
        <div class="kx-stat"><div class="kx-stat-icon icon-green"><i class="bi bi-check-circle-fill"></i></div>
            <div><div class="kx-stat-label">Approved</div><div class="kx-stat-value"><?php echo e($depApproved); ?></div></div></div>
        <div class="kx-stat"><div class="kx-stat-icon icon-blue"><i class="bi bi-cash-coin"></i></div>
            <div><div class="kx-stat-label">Total Volume</div><div class="kx-stat-value" style="font-size:1rem">₦<?php echo e(number_format($depTotal,0)); ?></div></div></div>
    </div>
    ...
    <div class="kx-panel">
        ...
        <div class="kx-table-wrap">
            <table class="kx-table" id="dTable">
                ...
                <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $deposits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dep): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                    $gatewayMeta = [];
                ...
                ?>
                <tr>
                    <td><span style="color:var(--kx-muted)">#<?php echo e($dep->id); ?></span></td>
                    <td><span style="font-weight:600"><?php echo e($dep->user->name ?? 'N/A'); ?></span><br>
                        <span style="font-size:.72rem;color:var(--kx-muted)"><?php echo e($dep->user->email ?? ''); ?></span></td>
                    <td><span style="font-weight:700;color:var(--kx-blue)">₦<?php echo e(number_format($dep->amount, 2)); ?></span></td>
                    <td style="font-size:.78rem">
                        <?php if($dep->payment_method === 'crypto_transfer'): ?>
                            <strong><?php echo e($gatewayMeta['crypto_wallet_name'] ?? 'Crypto Wallet'); ?></strong><br>
                            <span style="color:var(--kx-muted)"><?php echo e($gatewayMeta['crypto_wallet_network'] ?? 'Network'); ?></span><br>
                            <span style="font-family:monospace;color:var(--kx-muted)"><?php echo e($gatewayMeta['crypto_wallet_address'] ?? 'N/A'); ?></span>
                        <?php elseif($dep->companyAccount): ?>
                            <?php echo e($dep->companyAccount->bank_name); ?><br>
                            <span style="font-family:monospace;color:var(--kx-muted)"><?php echo e($dep->companyAccount->account_number); ?></span>
                        <?php else: ?>
                            <span style="color:var(--kx-muted)">—</span>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:.75rem;color:var(--kx-muted);white-space:nowrap">
                        <div><?php echo e(optional($dep->created_at)->format('d M Y')); ?></div>
                        <div style="font-size:.68rem;color:var(--kx-muted)"><?php echo e(optional($dep->created_at)->format('h:i A')); ?></div>
                    </td>
                    <td>
                        <?php if($dep->proof_of_payment): ?>
                            ...
                            ?>
                            <a href="<?php echo e($proofUrl); ?>" target="_blank" class="btn-kx-edit" style="font-size:.72rem;padding:.25rem .55rem">
                    ...
                    <td>
                        <?php if(in_array($dep->status, ['approved','successful','completed'])): ?>
                        ...
                        <?php elseif($dep->status === 'cancelled'): ?>
                            <span class="kx-badge kx-badge-red">Cancelled</span>
                        <?php else: ?>
                            <span class="kx-badge kx-badge-gray"><?php echo e($dep->status); ?></span>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:.75rem;color:var(--kx-muted)"><?php echo e($dep->admin_note ?? '—'); ?></td>
                    <td>
                        <form id="dep-form-<?php echo e($dep->id); ?>" action="<?php echo e(route('admin.deposits.update.post', $dep)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <div style="display:flex;gap:.4rem;align-items:center;flex-wrap:wrap">
                                <select name="status" id="dep-sel-<?php echo e($dep->id); ?>" class="kx-input" style="width:110px;padding:.3rem .5rem!important;font-size:.75rem!important">
                                    <option value="approved"  <?php echo e($dep->status === 'approved'  ? 'selected':''); ?>>Approve</option>
                                    <option value="cancelled" <?php echo e($dep->status === 'cancelled' ? 'selected':''); ?>>Cancel</option>
                                </select>
                                <input type="text" name="admin_note" id="dep-note-<?php echo e($dep->id); ?>" class="kx-input" style="width:120px" placeholder="Note…" value="<?php echo e($dep->admin_note); ?>">
                                <button type="button"
                                    class="btn-kx-green"
                                    style="padding:.3rem .65rem;font-size:.72rem"
                                    onclick="openDepConfirm(<?php echo e($dep->id); ?>,'<?php echo e(addslashes($dep->user->name ?? 'N/A')); ?>','<?php echo e(number_format($dep->amount, 2)); ?>','<?php echo e(addslashes($dep->payment_method ?? 'Bank Transfer')); ?>')"
                                    title="Review &amp; Save">
...
<?php echo $__env->make('adminnavlayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\kayxchangev2\resources\views\admin\deposits\index.blade.php ENDPATH**/ ?>