...
<div class="container" style="padding:1.25rem 0 3rem;">
    ...
    <div class="kx-hist-hero">
        ...
        <a href="<?php echo e(route('transactions.history', array_merge(request()->query(), ['export' => 'csv']))); ?>"
           class="kx-export-btn">
    ...
    <div class="kx-stat-row">
        <div class="kx-stat-pill">
            ...
            <div class="kx-stat-info"><div class="lbl">Total</div><div class="val"><?php echo e($allTrades->total()); ?></div></div>
        </div>
        <div class="kx-stat-pill">
            <div class="kx-stat-dot pend"></div>
            <div class="kx-stat-info"><div class="lbl">Pending</div><div class="val"><?php echo e($allTrades->getCollection()->where('status','pending')->count()); ?></div></div>
        </div>
        <div class="kx-stat-pill">
            <div class="kx-stat-dot ok"></div>
            <div class="kx-stat-info"><div class="lbl">Completed</div><div class="val"><?php echo e($allTrades->getCollection()->whereIn('status',['completed','approved'])->count()); ?></div></div>
        </div>
        <div class="kx-stat-pill">
            <div class="kx-stat-dot fail"></div>
            <div class="kx-stat-info"><div class="lbl">Failed</div><div class="val"><?php echo e($allTrades->getCollection()->whereIn('status',['failed','rejected','cancelled'])->count()); ?></div></div>
        </div>
    </div>

    
    <div class="kx-type-tabs">
        <a href="<?php echo e(route('transactions.history', array_merge(request()->except('type'), []))); ?>"
           class="kx-tab <?php echo e($activeType==='' ? 'active' : ''); ?>">
            <i class="bi bi-grid-fill"></i> All
        </a>
        <a href="<?php echo e(route('transactions.history', array_merge(request()->except('type'), ['type'=>'buy']))); ?>"
           class="kx-tab <?php echo e($activeType==='buy' ? 'active' : ''); ?>">
            <i class="bi bi-arrow-down-circle-fill"></i> Buy
        </a>
        <a href="<?php echo e(route('transactions.history', array_merge(request()->except('type'), ['type'=>'sell']))); ?>"
           class="kx-tab <?php echo e($activeType==='sell' ? 'active' : ''); ?>">
            <i class="bi bi-arrow-up-circle-fill"></i> Sell
        </a>
        <a href="<?php echo e(route('transactions.history', array_merge(request()->except('type'), ['type'=>'withdrawal']))); ?>"
           class="kx-tab <?php echo e($activeType==='withdrawal' ? 'active' : ''); ?>">
    ...
    <div class="kx-filter-panel">
        <div class="kx-filter-toggle <?php echo e($hasFilter ? 'open' : ''); ?>" id="filterToggle">
            <h6>
                <i class="bi bi-sliders" style="color:var(--kx-green)"></i>
                Filters
                <span class="kx-filter-dot <?php echo e($hasFilter ? 'active' : ''); ?>" id="filterDot"></span>
            </h6>
            <i class="bi bi-chevron-down chevron <?php echo e($hasFilter ? 'open' : ''); ?>" id="filterChevron"></i>
        </div>
        <div class="kx-filter-body <?php echo e($hasFilter ? 'open' : ''); ?>" id="filterBody">
            <form method="GET" action="<?php echo e(route('transactions.history')); ?>" id="filterForm">
                <?php if($activeType): ?><input type="hidden" name="type" value="<?php echo e($activeType); ?>"><?php endif; ?>
                <div class="row g-2 mb-2">
                    <div class="col-md-4 col-sm-6">
                        <input type="text" name="search" class="kx-finput" placeholder="Search coin, ref…" value="<?php echo e(request('search')); ?>">
                    </div>
                    ...
                    <div class="col-md-2 col-sm-6">
                        <input type="date" name="date" class="kx-finput" value="<?php echo e(request('date')); ?>">
                    </div>
                    <div class="col-md-2 col-sm-6">
                        <input type="number" name="min_amount" class="kx-finput" placeholder="Min ₦" value="<?php echo e(request('min_amount')); ?>">
                    </div>
                    <div class="col-md-2 col-sm-6">
                        <input type="number" name="max_amount" class="kx-finput" placeholder="Max ₦" value="<?php echo e(request('max_amount')); ?>">
                    </div>
                </div>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <a href="<?php echo e(route('transactions.history', $activeType ? ['type'=>$activeType] : [])); ?>"
    ...
    <div class="kx-table-wrap kx-desktop-only">
        <table class="kx-table">
            ...
            <tbody>
            <?php $__empty_1 = true; $__currentLoopData = $allTrades; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trade): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                <tr>
                    <td>
                        <span class="kx-type-badge <?php echo e($typClass); ?>">
                            <i class="bi <?php echo e($typIcon); ?>"></i>
                            <?php echo e(ucfirst($trade->type)); ?>

                        </span>
                    </td>
                    <td style="font-weight:600;color:#fff;"><?php echo e($trade->coin ?? '—'); ?></td>
                    <td><span class="kx-amt">₦<?php echo e(number_format($trade->naira_amount ?? $trade->amount, 2)); ?></span></td>
                    <td><span class="kx-amt-usd"><?php echo e($trade->usd_amount ? '$'.number_format($trade->usd_amount,2) : '—'); ?></span></td>
                    <td>
                        <span class="kx-sts <?php echo e($stsClass); ?>">
                            <?php if(in_array($trade->status,['completed','approved'])): ?><i class="bi bi-check-circle-fill"></i>
                            <?php elseif($trade->status==='pending'): ?><i class="bi bi-hourglass-split"></i>
                            <?php elseif(in_array($trade->status,['failed','rejected'])): ?><i class="bi bi-x-circle-fill"></i>
                            <?php else: ?><i class="bi bi-dash-circle"></i><?php endif; ?>
                            <?php echo e(ucfirst($trade->status)); ?>

                        </span>
                    </td>
                    <td style="color:var(--kx-muted);font-size:.78rem;"><?php echo e(Str::limit($methodLabel, 28)); ?></td>
                    <td style="max-width:170px;">
                        <span class="kx-ref" title="<?php echo e($walletLabel); ?>"><?php echo e(Str::limit($walletLabel, 26)); ?></span>
                    </td>
                    <td><span class="kx-ref"><?php echo e($trade->reference ?? '—'); ?></span></td>
                    <td style="color:var(--kx-muted);font-size:.78rem;white-space:nowrap;"><?php echo e($trade->created_at->format('d M Y, H:i')); ?></td>
                    <td>
                        <button class="kx-preview-btn" onclick="openPreview('<?php echo e($trade->type); ?>', <?php echo e($trade->id); ?>)">
    ...
    <?php $__empty_1 = true; $__currentLoopData = $allTrades; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trade): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        ...
        <div class="kx-tx-card">
            <div class="kx-tx-card-top">
                <span class="kx-type-badge <?php echo e($typClass); ?>">
                    <i class="bi <?php echo e($typIcon); ?>"></i> <?php echo e(ucfirst($trade->type)); ?>

                </span>
                <span class="kx-sts <?php echo e($stsClass); ?>">
                    <?php if(in_array($trade->status,['completed','approved'])): ?><i class="bi bi-check-circle-fill"></i>
                    <?php elseif($trade->status==='pending'): ?><i class="bi bi-hourglass-split"></i>
                    <?php else: ?><i class="bi bi-x-circle-fill"></i><?php endif; ?>
                    <?php echo e(ucfirst($trade->status)); ?>
            ...
            <div class="kx-tx-card-mid">
                <div>
                    <div style="font-size:.82rem;font-weight:700;color:#fff;">₦<?php echo e(number_format($trade->naira_amount ?? $trade->amount, 2)); ?></div>
                    <?php if($trade->usd_amount): ?><div class="kx-amt-usd">$<?php echo e(number_format($trade->usd_amount,2)); ?></div><?php endif; ?>
                    <?php if(in_array($trade->type, ['buy','sell']) && !empty($trade->wallet_address)): ?>
                        <div class="kx-ref" style="margin-top:.15rem;max-width:140px" title="<?php echo e($trade->wallet_address); ?>"><?php echo e(Str::limit($trade->wallet_address, 18)); ?></div>
                    <?php endif; ?>
                </div>
                <div style="text-align:right;">
                    <div style="font-size:.85rem;font-weight:600;color:#fff;"><?php echo e($trade->coin ?? '—'); ?></div>
                    <?php if($trade->reference): ?><div class="kx-ref"><?php echo e($trade->reference); ?></div><?php endif; ?>
                </div>
            </div>
            <div class="kx-tx-card-bot">
                <span class="kx-tx-date"><i class="bi bi-calendar3 me-1"></i><?php echo e($trade->created_at->format('d M Y, H:i')); ?></span>
                <button class="kx-preview-btn" onclick="openPreview('<?php echo e($trade->type); ?>', <?php echo e($trade->id); ?>)">
                    <i class="bi bi-eye-fill"></i> Preview
                </button>
            </div>
    ...
    <?php if($allTrades->hasPages()): ?>
    <div class="mt-4 d-flex justify-content-center">
        <?php echo e($allTrades->appends(request()->query())->links('pagination::bootstrap-5')); ?>
...
/* ── Drawer ── */
const detailUrl = '<?php echo e(url("/transactions/detail")); ?>';
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\kayxchangev2\resources\views/transactions/history.blade.php ENDPATH**/ ?>